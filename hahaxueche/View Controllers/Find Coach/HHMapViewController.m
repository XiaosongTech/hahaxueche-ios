...
- (void)sendLocationWithField:(HHField *)field coach:(HHCoach *)coach {
    ...
    view.buttonAction = ^(NSString *number) {
        ...
        [[HHStudentService sharedInstance] getPhoneNumber:number coachId:coachId schoolId:schoolId fieldId:field.fieldId eventType:@(5) eventData:eventData completion:^(NSError *error) {
            if (error) {
                [[HHToastManager sharedManager] showErrorToastWithText:@"提交失败, 请重试!"];
            } else {
