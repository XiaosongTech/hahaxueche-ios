...
- (void)drawRect:(CGRect)rect {
    ...
    CGContextSetRGBStrokeColor(context, 1.0f, 0.62f, 0.01f, 1);//线条颜色
    CGContextSetLineWidth(context, 2.0f/[UIScreen mainScreen].scale);
    CGContextMoveToPoint(context, 10.0f, rect.size.height);
    CGContextAddLineToPoint(context, rect.size.width-10.0f,rect.size.height);
...
- (void)setDataSource:(id<DOPDropDownMenuDataSource>)dataSource {
    ...
    for (int i = 0; i < _numOfMenu; i++) {
        ...
        //title
        CGPoint titlePosition = CGPointMake( (i * 2 + 1) * textLayerInterval-10.0f , self.frame.size.height / 2);
...
- (instancetype)initWithOrigin:(CGPoint)origin andHeight:(CGFloat)height {
    ...
    if (self) {
        ...
        _textColor = [UIColor HHLightTextGray];
...
- (CAShapeLayer *)createIndicatorWithColor:(UIColor *)color andPosition:(CGPoint)point {
    ...
    [path addLineToPoint:CGPointMake(8, 4)];
    
    layer.path = path.CGPath;
    layer.fillColor = nil;
    layer.lineWidth = 1;
    layer.strokeColor = [UIColor HHLightestTextGray].CGColor;
...
- (void)animateIndicator:(CAShapeLayer *)indicator Forward:(BOOL)forward complete:(void(^)())complete {
    ...
    if (forward) {
        // 展开
        indicator.strokeColor = [UIColor HHOrange].CGColor;
    } else {
        // 收缩
        indicator.strokeColor = [UIColor HHLightestTextGray].CGColor;
    }
